...
@onready var label_no_mony = $NoTeAlcanza
...
func _process(delta: float) -> void:
	if player_cerca and Input.is_action_just_pressed("interact") and ui_open == false:
		abrir()
...
func _on_aceptar_pressed() -> void:
	if player.coins <= 1:
		label_no_mony.visible = true
		$NoTeAlcanza/tiempodevida.start()
	if player.coins >= 2:
		player.add_coin(-2)
		player.health += 20
...
func _on_tiempodevida_timeout() -> void:
	label_no_mony.visible = false
